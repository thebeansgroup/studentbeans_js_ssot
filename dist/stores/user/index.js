// Generated by CoffeeScript 1.10.0
(function() {
  var AppDispatcher, Assign, CHANGE_EVENT, Constants, EventEmitter, Flux, UserStore, Yaks, _fetchDetails, _fetchStatus, _setReverifyState, _user, details_url, ref, ref1, status_url;

  Flux = require('tbg-flux-factory');

  AppDispatcher = require('../../dispatcher/');

  EventEmitter = require('events').EventEmitter;

  Assign = require('object-assign');

  Yaks = require('yaks');

  Constants = require('../../constants/');

  CHANGE_EVENT = 'change';

  _user = {
    role: "unregistered_users",
    status: "not_logged_in",
    flash: [],
    details: {}
  };

  window._user = _user;

  status_url = (ref = window._stb) != null ? ref.url_manifest.verge_status_path : void 0;

  details_url = (ref1 = window._stb) != null ? ref1.url_manifest.join_simple_details_path : void 0;

  _fetchStatus = function() {
    var VergeStore, params, req, reverify;
    Flux = require('tbg-flux-factory');
    VergeStore = Flux.getStore('Verge');
    reverify = VergeStore.getStateValue('reverify');
    params = {
      reverify: reverify
    };
    req = Yaks.UTILS.request({
      url: status_url,
      type: 'json',
      contentType: 'application/json',
      data: params
    });
    return req.then(function(resp) {
      var ref2;
      _user.role = resp.role;
      _user.status = resp.status;
      _user.flash = ((ref2 = resp.meta) != null ? ref2.flash : void 0) || [];
      return UserStore.emitChange();
    });
  };

  _fetchDetails = function() {
    var VergeStore, params, req, reverify;
    Flux = require('tbg-flux-factory');
    VergeStore = Flux.getStore('Verge');
    reverify = VergeStore.getStateValue('reverify');
    params = {
      reverify: reverify
    };
    req = Yaks.UTILS.request({
      url: details_url,
      type: 'json',
      contentType: 'application/json',
      data: params
    });
    return req.then(function(resp) {
      _user.details = resp;
      return UserStore.emitChange();
    });
  };

  _setReverifyState = function(data) {
    var store_data;
    return store_data = {
      reverify: data
    };
  };

  UserStore = Assign({}, EventEmitter.prototype, {
    getAll: function() {
      return _user;
    },
    getRole: function() {
      return _user.role;
    },
    getStatus: function() {
      return _user.status;
    },
    getDetails: function() {
      return _user.details;
    },
    getFlash: function() {
      return _user.flash;
    },
    emitChange: function() {
      return this.emit(CHANGE_EVENT);
    },
    addChangeListener: function(callback) {
      return this.on(CHANGE_EVENT, callback);
    },
    removeChangeListener: function(callback) {
      return this.removeListener(CHANGE_EVENT, callback);
    },
    dispatcherIndex: AppDispatcher.register(function(payload) {
      var action;
      action = payload.action;
      switch (action.actionType) {
        case Constants.USER_FETCH_STATUS:
          _fetchStatus(action.data);
          break;
        case Constants.USER_FETCH_DETAILS:
          _fetchDetails(action.data);
          break;
        case Constants.USER_SET_REVERIFY_STATE:
          _setReverifyState(action.data);
          break;
      }
      return true;
    })
  });

  module.exports = UserStore;

}).call(this);
